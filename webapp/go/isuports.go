...
func Run() {
	...
	idgen, _ = katsubushi.NewGenerator(1)
...
func playerHandler(c echo.Context) error {
	...
	cs := []CompetitionRow{}
	competitionTitle := make(map[string]string)
	if err := tenantDB.SelectContext(
		ctx,
		&cs,
	...
	for _, ps := range pss {
		psds = append(psds, PlayerScoreDetail{
			CompetitionTitle: competitionTitle[ps.CompetitionID],
			Score:            ps.Score,
		})
	}
...
func competitionRankingHandler(c echo.Context) error {
	...
	playersMap, err := retrievePlayersMap(ctx, tenantDB, playerIDs)
	if err != nil {
		return err
	}
	...
	for i, rank := range ranks {
		...
		pagedRanks = append(pagedRanks, CompetitionRank{
			...
			PlayerDisplayName: playersMap[rank.PlayerID].DisplayName,
		})
