...
var (
	checkNameByInstanceName = map[string]string{
		"isucon12-qualify-1": "qualify1",
		"isucon12-qualify-2": "qualify2",
		"isucon12-qualify-3": "qualify3",
	}
	privateAddressByInstanceName = map[string]string{
		"isucon12-qualify-1": "192.168.0.11",
		"isucon12-qualify-2": "192.168.0.12",
		"isucon12-qualify-3": "192.168.0.13",
	}
)
...
func (c *checker) checkInstance(i *ec2.Instance) {
	...
	if *i.InstanceType != c.ExpectedInstanceType {
		c.addFailure("%s のインスタンスタイプが %s です (%s である必要があります)", id, *i.InstanceType, c.ExpectedInstanceType)
	}
